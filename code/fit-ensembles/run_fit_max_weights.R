...
last_forecast_date <- lubridate::ymd("2022-01-03")
...
analysis_combinations <- tidyr::expand_grid(
  ...
  horizon_group = "all",
  max_weight = c("0.5", "0.4", "0.3", "0.2", "0.1")
) %>%
...
analysis_combinations$forecast_filename
...
if (run_setting %in% c("local", "midas_cluster_single_node")) {
  ...
  run_status <- #foreach(row_ind = seq_len(rev(nrow(analysis_combinations)))) %dopar% {
    ...
    max_weight <- analysis_combinations$max_weight[row_ind]
...
#    cat("module load gurobi/900\n", file = filename, append = TRUE)
	...
	drop_anomalies, "-", horizon_group, "-", max_weight,
        ".Rout"),
      file = filename, append = TRUE)
