...
for (hub in "US") {
  ...
  # for (measure in c("deaths")) {
  # for (measure in c("cases")) {
    outliers <- readr::read_csv(
      paste0("code/data-anomalies/outliers-inc-", measure, "-", hub, ".csv"),
      col_types = cols(
    ...
    revisions <- readr::read_csv(
      paste0("code/data-anomalies/revisions-inc-", measure, "-", hub, ".csv"),
      col_types = cols(
        location = col_character(),
  ...
  #        date %in% combined_data$date
    ...
    readr::write_csv(
      revisions_to_drop,
      paste0("code/data-anomalies/revisions-to-drop-inc-", measure, "-", hub, ".csv")
    )
  }
}
